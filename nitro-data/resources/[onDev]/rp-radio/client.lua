...
-- CONNECTION
-----------------------------------------------------------------------------------------------------------------------------------------
vRPEX = Tunnel.getInterface("rp-radio")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local Radio = {
...
RegisterCommand('radio', function(source,args,rawCommand)
    if vRPEX.checkRadio() and GetEntityHealth(ped) <= 101 then
        if not args[1] then
            if not firstuse then
                ...
                if Radio.On then
                    ...
                    TriggerEvent("Notify","sucesso","Radio <b>ligado</b>.")
                else
                    Radio:Remove()
                    SendNUIMessage({ sound = "audio_off", volume = 0.5})
                    TriggerEvent("Notify","sucesso","Radio <b>desligado</b>.")
                end
            else 
                TriggerEvent("Notify","sucesso","Coloque uma <b>frequencia</b>.")
            end
        else
            if checkPermission(tonumber(args[1])) then
                firstuse = false
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- SETUP EACH RADIO COMMAND IF ENABLE
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- CREATE/DESTRON HANDHELD RADIO OBJECT
-----------------------------------------------------------------------------------------------------------------------------------------
...
function checkPermission(id)
    if id >= 1000 then
        if parseInt(id) == 1000 and vRPEX.checkPermission("vipfree.permissao","Suporte") then
            return true
        end
        if parseInt(id) == 1001 and vRPEX.checkPermission("suporte.permissao","Staff") then
            return true
        end
        if parseInt(id) == 1002 and vRPEX.checkPermission("paramedico.permissao","Hospital") then
            return true
        end
        if parseInt(id) == 1003 and vRPEX.checkPermission("policia.permissao","VTR01") then
            return true
        end
        if parseInt(id) == 1004 and vRPEX.checkPermission("policia.permissao","VTR02") then
            return true
        end
        if parseInt(id) == 1005 and vRPEX.checkPermission("policia.permissao","COPOM") then
            return true
        end
        if parseInt(id) == 1006 and vRPEX.checkPermission("bope.permissao","Bope") then
            return true
        end
        if parseInt(id) == 1007 and vRPEX.checkPermission("pfederal.permissao","Policia Federal") then
            return true
        end
        if parseInt(id) == 1008 and vRPEX.checkPermission("pfederal.permissao","Investigacao") then
            return true
        end
        if parseInt(id) == 1009 and vRPEX.checkPermission("mecanico.permissao","Lotus") then
            return true
        end
        if parseInt(id) == 1010 and vRPEX.checkPermission("mecanico.permissao","Bennys") then
            return true
        end
        if parseInt(id) == 1011 and vRPEX.checkPermission("milicia.permissao","Milicia") then
            return true
        end
        if parseInt(id) == 1012 and vRPEX.checkPermission("motoclub.permissao","Motoclub") then
            return true
        end
        if parseInt(id) == 1013 and vRPEX.checkPermission("mafia.permissao","Mafia") then
            return true
        end
        if parseInt(id) == 1014 and vRPEX.checkPermission("yakuza.permissao","Yakuza") then
            return true
        end
        if parseInt(id) == 1015 and vRPEX.checkPermission("cv.permissao","CV") then
            return true
        end
        if parseInt(id) == 1016 and vRPEX.checkPermission("pcc.permissao","PCC") then
            return true
        end
		if parseInt(id) == 1017 and vRPEX.checkPermission("tcp.permissao","TCP") then
            return true
        end
		if parseInt(id) == 1018 and vRPEX.checkPermission("cosanostra.permissao","Cosanostra") then
            return true
        end
		if parseInt(id) == 1019 and vRPEX.checkPermission("ada.permissao","ADA") then
            return true
        end
		if parseInt(id) == 1020 and vRPEX.checkPermission("vanilla.permissao","Vanilla") then
            return true
        end
    else
        return true
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- SET IF PLAYER HAS ACESS TO USE THE RADIO WHEN CLOSED
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- GIVE ACESS TO MULTIPLE FREQUENCIES
-----------------------------------------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- REMOVE ACESS TO MULTIBLE FREQUENCIES
-----------------------------------------------------------------------------------------------------------------------------------------
