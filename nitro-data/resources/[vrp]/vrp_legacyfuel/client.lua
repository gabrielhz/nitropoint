...
AddEventHandler('fuel:startFuelUpTick',function(pumpObject,ped,vehicle)
	...
	if pumpObject then
		TriggerServerEvent('vrp_gasolinafuel:pagamento',parseInt(currentCost))
	end

	currentCost = 0.0
end)

RegisterNetEvent('vrp_gasolinafuel:insuficiente')
AddEventHandler('vrp_gasolinafuel:insuficiente',function()
...
RegisterNetEvent('vrp_gasolinafuel:galao')
AddEventHandler('vrp_gasolinafuel:galao',function()
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped),-1) == ped then
				...
				elseif isNearPump then
					...
					if IsControlJustReleased(0,38) then
						TriggerServerEvent('vrp_gasolinafuel:pagamento',parseInt(300),true)
					end
				else
					Citizen.Wait(250)
				end
