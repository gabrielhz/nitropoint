...
function vRPdmv.setLicense()
	...
	vRP.setUData(user_id,"vRP:jyz:license",json.encode(os.date("%x")))
end
...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
	local user_id = vRP.getUserId(source)
	local player = vRP.getUserSource(user_id)
	local data = vRP.getUData(user_id,"vRP:jyz:license")
	if data then
...
--[[RegisterCommand("cnh",function(source)
  ...
  if vRP.hasPermission(user_id,"policia.permissao") or vRP.hasPermission(user_id,"cnh.permissao") then
    if nuser_id then
      ...
      local data = vRP.getUData(nuser_id,"vRP:jyz:license")
        if data then
        local license = json.decode(data)
      if license and license ~= 0 then
  ...
  if vRP.hasPermission(user_id,"policia.permissao") then
    if nuser_id then
      ...
      if license and license ~= 0 then
          ...
          vRP.setUData(nuser_id,"vRP:jyz:license",json.encode())
