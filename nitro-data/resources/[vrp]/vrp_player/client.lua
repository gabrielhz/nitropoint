...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(1,  118) then
            TriggerServerEvent("onblue:qthPolice")
        end
    end
end)
...
RegisterCommand("attachs",function(source,args)
	...
	elseif GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_ASSAULTRIFLE_MK2") then
		...
		GiveWeaponComponentToPed(ped,GetHashKey("WEAPON_ASSAULTRIFLE_MK2"),GetHashKey("COMPONENT_AT_SCOPE_MACRO"))
		GiveWeaponComponentToPed(ped,GetHashKey("WEAPON_ASSAULTRIFLE_MK2"),GetHashKey("COMPONENT_AT_AR_AFGRIP"))
