...
Citizen.CreateThread(function()
	TriggerServerEvent("FacilAdmin:amiadmin")
	TriggerServerEvent("FacilAdmin:requestBanlist")
	...
	mainMenu = NativeUI.CreateMenu("FacilAdmin", "~b~Admin Menu", menuOrientation, 0)
...
function GenerateMenu() -- this is a big ass function
	...
	for i,thePlayer in ipairs(players) do
		...
		if permissions.kick then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("FacilAdmin:kickPlayer", GetPlayerServerId( thePlayer ), KickReason)
				BanTime = 1
				BanReason = ""
		...
		if permissions.ban then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("FacilAdmin:banPlayer", GetPlayerServerId( thePlayer ), BanReason, banLength[BanTime].time, GetPlayerName( thePlayer ))
				BanTime = 1
				BanReason = ""
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
		...
		if permissions.mute then			
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("FacilAdmin:mutePlayer", GetPlayerServerId( thePlayer ))
			end
		end
		...
		if permissions.spectate then
			...
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("FacilAdmin:requestSpectate",thePlayer)
			end
		end
		...
		if permissions.teleport then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				...
				TriggerServerEvent("FacilAdmin:TeleportPlayerToCoords", GetPlayerServerId(thePlayer), px,py,pz)
			end
		end
		...
		if permissions.slap then
			...
			thisItem.OnSliderSelected = function(index)
				TriggerServerEvent("FacilAdmin:SlapPlayer", GetPlayerServerId(thePlayer), index*10)
			end
		end
			...
			thisPlayer.OnListSelect = function(sender, item, index)
					if item == thisItem then
							...
							if i == GetLocalisedText("on") then
								TriggerServerEvent("FacilAdmin:FreezePlayer", GetPlayerServerId(thePlayer), true)
							else
								TriggerServerEvent("FacilAdmin:FreezePlayer", GetPlayerServerId(thePlayer), false)
							end
					end
			end
		end
		...
		if permissions.screenshot then
			...
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("FacilAdmin:TakeScreenshot", GetPlayerServerId(thePlayer))
			end
		end
	...
	if permissions.teleport then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			local px,py,pz = table.unpack(GetEntityCoords(PlayerPedId(),true))
			TriggerServerEvent("FacilAdmin:TeleportPlayerToCoords", -1, px,py,pz)
		end
	end
	...
	if permissions.manageserver then
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("FacilAdmin:SetGameType", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("FacilAdmin:SetMapName", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				TriggerServerEvent("FacilAdmin:StartResource", result)
			end
		end
		...
		thisItem.Activated = function(ParentMenu,SelectedItem)
			...
			if result then
				if result ~= GetCurrentResourceName() and result ~= "NativeUI" then
					TriggerServerEvent("FacilAdmin:StopResource", result)
				else
					TriggerEvent("chat:addMessage", { args = { "FacilAdmin", GetLocalisedText("badidea") } })
				end
			end
		end
	...
	if permissions.unban then
		...
		for i,theBanned in ipairs(banlist) do
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("FacilAdmin:unbanPlayer", identifier)
				TriggerServerEvent("FacilAdmin:requestBanlist")
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
				GenerateMenu()
	...
	if permissions.unban then
		...
		settingsMenu:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent("FacilAdmin:updateBanlist")
		end
	end
	...
	if permissions.anon then
		...
		settingsMenu.OnCheckboxChange = function(sender, item, checked_)
			if item == thisItem then
				anonymous = checked_
				TriggerServerEvent("FacilAdmin:SetAnonymous", checked_)
			end
		end
	end
