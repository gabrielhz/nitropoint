local discord_webhook = "https://discordapp.com/api/webhooks/730054635246714951/kbUQ4vKqY-7cIV-xHkmhCC8AKeI21nHmwqRKDGSUW-66QgcicrpXyLjJuG7dA2hFwrq5"
local discord_webhook1 = "https://discordapp.com/api/webhooks/730054635246714951/kbUQ4vKqY-7cIV-xHkmhCC8AKeI21nHmwqRKDGSUW-66QgcicrpXyLjJuG7dA2hFwrq5"
local discord_webhook2 = "https://discordapp.com/api/webhooks/730054635246714951/kbUQ4vKqY-7cIV-xHkmhCC8AKeI21nHmwqRKDGSUW-66QgcicrpXyLjJuG7dA2hFwrq5"
local discord_webhook3 = "https://discordapp.com/api/webhooks/730054635246714951/kbUQ4vKqY-7cIV-xHkmhCC8AKeI21nHmwqRKDGSUW-66QgcicrpXyLjJuG7dA2hFwrq5"
local discord_webhook4 = "https://discordapp.com/api/webhooks/730054635246714951/kbUQ4vKqY-7cIV-xHkmhCC8AKeI21nHmwqRKDGSUW-66QgcicrpXyLjJuG7dA2hFwrq5"
local discord_webhook5 = "https://discordapp.com/api/webhooks/730054635246714951/kbUQ4vKqY-7cIV-xHkmhCC8AKeI21nHmwqRKDGSUW-66QgcicrpXyLjJuG7dA2hFwrq5"
...
local itemlist = {
	...
	["radio"] = { index = "radio", nome = "Radinho" },
	...
	["wammo|WEAPON_ASSAULTRIFLE_MK2"] = { index = "m-ak103", nome = "Munição de AK-103" },
...
RegisterCommand('item',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,"admin.permissao") then
		if args[1] and args[2] and itemlist[args[1]] ~= nil then
			...
			vRP.log("admin", "Give Item", "O administrador: "..user_id.."\nItem: "..args[1].."\nQuantidade:"..args[2])
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	for k,v in pairs(itemlist) do
		if args[1] == v.index and args[1] ~= "mochila" then
			...
			if vRP.tryGetInventoryItem(user_id,k,1) then
				...
				vRP.log("webhook", "Equipar","ID: "..user_id.."\nArma: "..v.index)			
			else
		...
		if args[1] == v.index and args[1] ~= "mochila" then
			...
			if uweapons[weaponuse] then
				...
				if vRP.tryGetInventoryItem(user_id,"wammo|"..weaponuse,parseInt(args[2])) then
					...
					vRP.log("webhook", "Recarregar", "ID: "..user_id.."\nArma: "..v.index)		
				else
			...
			if args[1] == v.index then
				if args[2] and parseInt(args[2]) > 0 then
					if vRP.tryGetInventoryItem(user_id,k,parseInt(args[2])) then
						...
						vRP.log("webhook", "Dropar","ID: "..user_id.."\nItem: "..k.. "\nQuantidade: "..parseInt(args[2]))		
					end
				else
					local data = vRP.getUserDataTable(user_id)
...
AddEventHandler("kickAFK",function()
	...
	if not vRP.hasPermission(user_id,"admin.permissao") then
		...
		vRP.log("kickban", "Kick AFK","ID: "..user_id)		
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- /SEQUESTRO
...
RegisterCommand('enviar',function(source,args,rawCommand)
			...
			if args[1] == v.index then
				if vRP.getInventoryWeight(nuser_id)+vRP.getItemWeight(k)*parseInt(args[2]) <= vRP.getInventoryMaxWeight(nuser_id) then
					if vRP.tryGetInventoryItem(user_id,k,parseInt(args[2])) then
						...
						vRP.log("webhook", "Enviou","De: "..user_id.." Para: "..nuser_id.."\nItem: "..k.. "\nQuantidade: "..args[2])
					end
				end
			end
		end
	elseif nuser_id and parseInt(args[1]) > 0 then
		if vRP.tryPayment(user_id,parseInt(args[1])) then
			...
			vRP.log("dinheiro", "Enviou R$","De: "..user_id.." Para: "..nuser_id.. "\nQuantidade: R$"..args[1])
		else
			...
			if vRP.request(nplayer,"Você está sendo roubado, deseja passar tudo?",30) then
				...
				if vida <= 100 then
					...
					SetTimeout(30000,function()
						...
						vRP.log("roubos", "Roubo","Infrator: "..user_id.." Vítima: "..nuser_id.."\n"..nmoney)
					end)
				else
					local ndata = vRP.getUserDataTable(nuser_id)
					if ndata ~= nil then
...
RegisterCommand('chamar',function(source,args,rawCommand)
  ...
  --[[  local content3 = "```Player: "..user_id.." \nFez um chamado para: "..args[1].."\nMensagem: "..descricao.."```"
		PerformHttpRequest(discord_webhook2, function(err, text, headers) end, 'POST', json.encode({username = "Log's", content = content3}), { ['Content-Type'] = 'application/json' })
		]]--
		vRP.log("chamados", "Chamado","ID: "..user_id.."\nFez um chamado para: "..args[1].."\nMensagem: "..descricao)
...
RegisterServerEvent('onblue:qthPolice')
AddEventHandler('onblue:qthPolice', function()
	local source = source
	local user_id = vRP.getUserId(source)
	local uplayer = vRP.getUserSource(user_id)
	local identity = vRP.getUserIdentity(user_id)
		...
		if data.inventory then			
			...
			local equip = function(player, choice)				
				...
				if vRP.tryGetInventoryItem(user_id,itemSelected,1) then
					...
					vRP.log("webhook", "Equipar","ID: "..user_id.."\nArma: "..itemlist[itemSelected].index)
						...
						if itemlist[itemSelected].index == v.index then
							if vRP.getInventoryWeight(nuser_id)+vRP.getItemWeight(k)*parseInt(quantity) <= vRP.getInventoryMaxWeight(nuser_id) then
								if vRP.tryGetInventoryItem(user_id,k,parseInt(quantity)) then
									...
									vRP.log("webhook", "Enviar Item","ID: "..user_id.." Para: "..nuser_id.."\nItems:\n"..k)
								end
							end
						end
					end			
...
--[[RegisterCommand('me',function(source,args,rawCommand)
	if args[1] then
		...
		--SendWebhookMessage(webhookme,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[/ME]: "..rawCommand:sub(3).." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
	end
end)]]
-----------------------------------------------------------------------------------------------------------------------------------------
-- CARTAS
...
RegisterCommand('use',function(source,args,rawCommand)
	...
	vRP.log("webhook", "Usar Item","ID: "..user_id.."\nItem:"..args[1])
...
RegisterCommand('paypal',function(source,args,rawCommand)
	...
	if user_id then
		if args[1] == "sacar" and parseInt(args[2]) > 0 then
			...
			if confirmacao == "Sim" or "sim" then
			if resultado >= parseInt(args[2]) then
				...
				vRP.log("dinheiro", "Paypal Sacar","ID: "..user_id.."\nSacou: R$"..vRP.format(parseInt(args[2])))
			else
		...
		elseif args[1] == "trans" and parseInt(args[2]) > 0 and parseInt(args[3]) > 0 then
			if parseInt(args[2]) == user_id then
			vRP.log("antijogo", "Paypal Bugar"," "..identity.name.." "..identity.firstname.."\nID: "..user_id.." \nIP: "..GetPlayerEndpoint(source).."\nTentou bugar o paypal e foi kickado.\n"..vRP.format(parseInt(args[3])))
			SendWebhookMessage(webhookteste,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[IP]: "..GetPlayerEndpoint(source).." \n[BUGOU]: $"..vRP.format(parseInt(args[3])).." \nVai lá dar um BAN neste babaca! "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
			vRP.kick(source,"Vai bugar na casa do caralho! Uma LOG foi enviada para a STAFF e logo logo seu ban chega! :)")
			return
			end
			...
			if vRP.request(source,"Deseja transferir <b>$"..vRP.format(parseInt(args[3])).."</b> dólares para <b>"..identityu.name.." "..identityu.firstname.."</b>?",30) then	
				if banco >= parseInt(args[3]) then
					...
					vRP.log("dinheiro", "Paypal Transferir","ID: "..user_id.."\nPara: "..vRP.format(parseInt(args[2])).."\nTransferiu: R$"..vRP.format(parseInt(args[3])))
