...
-- knut.RegisterServerCallback('loffe_heist:getCops', function(source, cb)
    -- local xPlayers = knut.GetPlayers()
    -- local cops = 0
    -- for i = 1, #xPlayers do
        -- if knut.GetPlayerFromId(xPlayers[i]).job.name == 'police' then
            -- cops = cops + 1
        -- end
    -- end
    -- print(cops)
...
AddEventHandler('loffe_heist:setDoorFreezeStatus', function(bank, door, status)
    ...
    -- else
        -- TriggerClientEvent('knut:showNotification', src, 'Det är inte tillräckligt med poliser online!') -- prob lua executor (or bug)
    -- end
end)
