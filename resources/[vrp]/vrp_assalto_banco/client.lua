            ...
            if HasScaleformMovieLoaded(scaleform) then
                ...
                if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                    ...
                    elseif Hacking and ProgramID == 86 then
                        ...
                        mHacking = false
                        -- TriggerServerEvent('knut_phone:send', 'police', 'Ett dataintrång har skett på ' .. v.BankName .. ' och bankvalvet har öppnats!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
                    ...
                    if Hacking then
                        ...
                        if lives <= 0 then
                            ...
                            Wait(5000)
                            local v = Config.BankRobbery[hackingBank]
                            -- TriggerServerEvent('knut_phone:send', 'police', 'En anonym person har försökt att göra intrång i ' .. v.BankName .. 's system!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
            ...
            if GetDistanceBetweenCoords(coords, v.Hacking.Coords, true) <= 2.0 and v.Doors[#v.Doors].Frozen and not mHacking then
                ...
                if allowed then
                    ...
                    if IsControlPressed(0, 38) then
                        local police = nil
                        -- knut.TriggerServerCallback('loffe_heist:getCops', function(cops)
                            -- police = 5
                        -- end)
                        -- while police == nil do
                            -- Wait(5)
...
-- RegisterNetEvent('knut:playerLoaded')
-- AddEventHandler('knut:playerLoaded', function(xPlayer)
    -- PlayerData = xPlayer
-- end)

-- RegisterNetEvent('knut:setJob')
-- AddEventHandler('knut:setJob', function(job)
    -- PlayerData.job = job
-- end)
...
Citizen.CreateThread(function()
    while true do
        -- while knut == nil do
            -- Wait(25)
        -- end
        -- while knut.GetPlayerData().inventory == nil do
            -- Wait(25)
        -- end
        -- if knut.GetPlayerData().inventory ~= nil then
            -- local Inventory = knut.GetPlayerData().inventory
...
-- Citizen.CreateThread(function()
	-- while true do
		-- Wait(250)
		-- if knut.GetPlayerData().job.name == "police" then
			-- for i = 1, #Config.BankRobbery do
