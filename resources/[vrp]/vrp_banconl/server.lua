...
AddEventHandler("vRPclient:playerSpawned",function(user_id,source) 
    ...
    TriggerClientEvent('iung:updateBalance', source, bankbalance, multasbalance, walletbalance)
end)
...
function Depositar(user_id, amount)
  ...
  TriggerClientEvent("iung:updateBalance", source, new_balance)
...
RegisterServerEvent('treak:update')
AddEventHandler('treak:update', function()
  local source = source
  local user_id = vRP.getUserId(source)
  local bankbalance = vRP.getBankMoney(user_id)
  ...
  TriggerClientEvent("iung:updateBalance", source, bankbalance, walletbalance, multasbalance, identity.name.." "..identity.firstname) -- ADICIONAR VALORES AQUI PARA APARECER NA NUI + JAVA SCRIPT + HTML
end)

RegisterServerEvent('treak:pagarmulta')
AddEventHandler('treak:pagarmulta', function(amount)
  ...
  if vRP.tryFullPayment(user_id,rounded) then
    ...
    TriggerClientEvent("iung:updateBalance", source, bank, wallet,novamulta)
    TriggerClientEvent("iung:removeMulta", source, rounded)
    ...
    if amount and type(amount) == "number" then
      ...
      if (rounded > 0) then
        ...
        if (rounded <= wallet) then
          ...
          TriggerClientEvent("iung:updateBalance", source, new_balance,carteira)
          TriggerClientEvent("iung:addBalance", source, rounded)
  ...
  if user_id then
    if amount and type(amount) == "number" then
      ...
      if rounded <= bankbalance then
        ...
        -- Update NUI
        TriggerClientEvent("iung:updateBalance", source, new_balance, carteira)
        TriggerClientEvent("iung:removeBalance", source, rounded)
        -- Salva o extrato
...
RegisterServerEvent('treak:quickCash')
AddEventHandler('treak:quickCash', function()
  local source = source
  local user_id = vRP.getUserId(source)
  ...
  if (bankbalance >= quantia) then
    ...
    local carteira = vRP.getMoney(user_id)
    TriggerClientEvent("iung:updateBalance", source, new_balance,carteira)
    TriggerClientEvent("iung:removeBalance", source, quantia)
...
AddEventHandler('treak:transfer', function(toPlayer, amount)
  local source = source
  local user_id = vRP.getUserId(source)
      ...
      if (rounded > 0) then
        ...
        if (rounded <= bankbalance) then
          ...
          if player then -- Está online
            ...
            TriggerClientEvent("iung:updateBalance", player, newBalance_Player)
            TriggerClientEvent("iung:addBalance", player, rounded)
          else
          ...
          local carteira = vRP.getMoney(user_id)
          TriggerClientEvent("iung:updateBalance", source, newBalance, carteira)
          TriggerClientEvent("iung:removeBalance", source, rounded)
          -- Extrato
          ...
          TriggerClientEvent("Notify", source, "sucesso","Você transferiu <b>$"..rounded.."</b> para o <b>Passaporte: "..nuser_id.."</b>")
          local nplayer = vRP.getUserSource(nuser_id)
					if nplayer == nil then
						return
					else
						local identity = vRP.getUserIdentity(user_id)
						TriggerClientEvent("Notify",player,"importante","<b>"..identity.name.." "..identity.firstname.."</b> transferiu <b>$"..rounded.." dólares</b> para sua conta do conta bancária.")
					end
