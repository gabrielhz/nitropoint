		...
		jQuery.each(data, function(key, value) {
			$('#garagem').append(`
				<div class="item" data-item-name="${value.vehicle}">
					<div class="item-img">
						<img src="img/${value.vehicle}.jpg" class="img"/>
					</div>
					<div class="item-text">
...
$(document).on('click','.item2',function(){
	...
	let isActive = $el.hasClass('active');
	if(isActive){
		$.post('http://vrp_garages/spawnVehicles2',JSON.stringify({name: $el.attr('data-item-name'), preco: $el.attr('data-item-price')}));
	}else{
		$('.item').removeClass('active');
		if(!isActive) $el.addClass('active');
		$.post('http://vrp_garages/notifyRent',JSON.stringify({name: $el.attr('data-item-name'), preco: $el.attr('data-item-price')}));
	}
});
...
$(document).on('click','#alugar',function(){
	let $el = $('.item.active');
	if($el) {
		$.post('http://vrp_garages/rentVehicles',JSON.stringify({name: $el.attr('data-item-name')}),(data) => {
			$('#garagem').html('');
			jQuery.each(data, function(key, value) {
				$('#garagem').append(`
